...
from networks.pt_fully_conv import FullyConv
...
class A2CAgent(object):
    ...
    def _mask_unavailable_actions(self, available_actions, fn_pi):
        fn_pi = fn_pi * available_actions   # masking multiply for removing unavailable action
        fn_pi = fn_pi / fn_pi.sum(1, keepdim=True) # redistribute probability after removing unavailable action case
        return fn_pi
    ...
    def _sample_actions(self, available_actions, policy):
        ...
        while True:
            ...
            if (available_actions.gather(1, fn_samples.unsqueeze(1)) == 1).all():  # check available action with selected action
