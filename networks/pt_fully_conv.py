...
from environ.pre_processing import is_spatial_action, NUM_FUNCTIONS
from environ.pre_processing import screen_specs_sv, minimap_specs_sv, flat_specs_sv
...
class FullyConv(object):
    def __init__(self, args, supervised=False):
        ...
        self.screen_out = nn.DataParallel(nn.Sequential(
                self._conv2d_init(35, 8, stride=1, kernel_size=5, padding=2),
                nn.ReLU(True),
                self._conv2d_init(8, 16, stride=1, kernel_size=3, padding=1),
                nn.ReLU(True)))
        self.minimap_out = nn.DataParallel(nn.Sequential(
                self._conv2d_init(12, 12, stride=1, kernel_size=5, padding=2),
                nn.ReLU(True),
