...
class Preprocess:
    def __init__(self):
        ...
        self.num_flat = features.Player
        self.available_actions_channels = arglist.NUM_ACTIONS

    def get_observation(self, state):

        obs_flat = state.observation['player']
        available_actions = state.observation['available_actions']
        available_actions = self._fill_available_actions(available_actions)
    ...
    def _fill_available_actions(self, x):
        y = np.zeros((self.available_actions_channels,), dtype='float32')
        y[x] = 1.
        return y
